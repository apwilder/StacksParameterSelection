...
#Define variables
R1DIR=$1 #/media/jamie/4TB/Pochard/ProcessRADtagsOutput_BP/R1/
R2DIR=$2 #/media/jamie/4TB/Pochard/ProcessRADtagsOutput_BP/R2/
OUTPUTMAIN=$3 #/media/jamie/4TB/Pochard/Stacks_DeNovo_Output/
POPMAP=$4 #/media/jamie/4TB/TAM_run01/Waterbuck_January2018/PopulationMap_excl.txt (tab-delimited table of sample ID, Population ID)
...
# nohup ./StacksDeNovoV2.sh /media/jamie/4TB/Pochard/ProcessRADtagsOutput_BP/R1/ \
# /media/jamie/4TB/Pochard/ProcessRADtagsOutput_BP/R2/ \
# /media/jamie/4TB/Pochard/Stacks_DeNovo_Output/ \
# /media/jamie/4TB/Pochard/PopulationMapTest.txt \
# 2 5 1 5 1 5 3 2 1 >& Stacks2_Pochard_m2-5_M1-5_n1-5_m3M2n1.nohup &
...
# /media/jamie/4TB/Pochard/Stacks_DeNovo_Output/ \
# /media/jamie/4TB/Pochard/PopulationMapTest.txt \
# 3 2 1 >& Stacks2_Pochard_m3M2n1.nohup &

##########################MAIN FUNCTION####################################################
StacksPipe () { #don't need this line if running code in piece-meal
...
i=`cat $POPMAP | grep -n $SAMPLE | cut -d: -f1` 
###disable use of secondary reads if m>6, as recommended by Paris et al. 2017
if [[ $m -gt 6 ]] #if >m6 disable use of secondary reads
then
ustacks -f $R1DIR$SAMPLE'.fq.gz' -i $i -o $OUTPUTDIR -M $M -m $m -p 24 -d -N 0 >& $OUTPUTDIR$ID'_ustack.out'
else #otherwise use secondary reads
ustacks -f $R1DIR$SAMPLE'.fq.gz' -i $i -o $OUTPUTDIR -M $M -m $m -p 24 -d >& $OUTPUTDIR$ID'_ustack.out'
fi
...
$(grep "Stack coverage: " $ID'_ustack.out' | tr '=;(' '\t' | cut -f2,4,6,8) $(grep "Blacklisted " $ID'_ustack.out' | tr -s ' ' | cut -f 3 -d' ' | tr ' ' '\t') \
$(grep "repeat removal: " $ID'_ustack.out' | tr '=;(' '\t' | cut -f 2,4,6,8) $(grep Assembled $ID'_ustack.out' | grep blacklisted | tr '; ' '\t' | cut -f4,7,10) \
...
cstacks -P $OUTPUTDIR -M $POPMAP -n $n -p 24 >& $OUTPUTDIR'Cstack.out'
...
sstacks -P $OUTPUTDIR -M $POPMAP -p 24 >& $OUTPUTDIR'Sstack.out' 

tsv2bam -P $OUTPUTDIR -M $POPMAP -t 24 -R $R2DIR # -b $n #convert to tsv files to bam

###Merge paired-end reads to contigs, align reads and call SNPs #############
gstacks -P $OUTPUTDIR -M $POPMAP -O $OUTPUTDIR -t 24

#### COMPUTE POPULATION STATISTICS USING POPULATIONS ####
populations -P $OUTPUTDIR -M $POPMAP -t 24 --write_single_snp --genepop --plink --vcf -r 0.50 --min_maf 0.02 #run populations at r=0.5 (output SNPs present in 50% of samples)
...
populations -P $OUTPUTDIR -M $POPMAP -t 24 --write_single_snp --genepop --plink --vcf -r 0.80 --min_maf 0.02 #run populations at r=0.8 (output SNPs present in 80% of samples)
sed -i 's/# Pop ID/Pop_ID/g' populations.sumstats_summary.tsv #edit an output file so it's easier to use downstream
